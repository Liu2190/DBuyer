...
-(void)createTimeOut:(NSNumber *)time
{
	[self performSelector:@selector(resleaseTimer)];
    self.timeOutTimer = [NSTimer scheduledTimerWithTimeInterval:[time intValue] target:self selector:@selector(timeOutMethod) userInfo:nil repeats:NO];
}
