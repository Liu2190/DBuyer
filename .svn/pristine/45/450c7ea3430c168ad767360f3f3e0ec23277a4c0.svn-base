...
- (void)viewDidLoad {
    ...
    [self.view makeInsetShadowWithRadius:1 Color:[UIColor grayColor]
...
- (void)genTabBarItmesByItemNames:(NSArray*)itemNames {
    ...
    for (int i=0;i<itemNames.count;i++) {
        ...
        TTabBarBlock *blockView = [[TTabBarBlock alloc]initWithFrame:CGRectMake(x, y, w, h)
                                                         andItemName:itemName andIndex:i];
...
- (void)updateLineOnSwitch:(int)index {
    ...
    moveAnimation = [[TUtilities getInstance]objectMoveFrom:CGPointMake(_lineImageView.center.x, _lineImageView.center.y) toPoint:center duration:.2f];
