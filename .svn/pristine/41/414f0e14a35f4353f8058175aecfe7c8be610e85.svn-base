...
@implementation TChargeAllscoController

- (id)initWithNavigationTitle:(NSString *)title andCard:(TAllScoCard*)card {
...
- (void)viewDidLoad {
    ...
    self.chargeFormController = [[TAllscoChargeFormController alloc]initWithCard:_card];
    CGRect rect = CGRectMake(0, 0, self.contentView.frame.size.width, self.contentView.frame.size.height);
    [_chargeFormController.view setFrame:rect];
    _chargeFormController.allscoChargeController = self;
    [self.contentView addSubview:_chargeFormController.view];
}

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    _chargeFormController.root.appearance.buttonAlignment = NSTextAlignmentCenter;
}

- (void)chargeAllscoByCharge:(TAllScoCharge*)allscoCharge {
    _allscoCharge = allscoCharge;
    ...
    [[TServerFactory getServerInstance:@"TAllScoServer"]doChargeCardByPhoneName:allscoCharge.phoneNumber
                                                                andChargeAmount:allscoCharge.chargeAmount
                                                                 andChargeCards:allscoCharge.chargeCards
                                                                        andFlag:allscoCharge.payIndexValue
                                                                      orderDate:orderDate
                                                                    andCallback:^(NSString *datas,NSString *umpData) {
                                                                        [[TUtilities getInstance]dismiss];
                                                                        NSString *paraValue = datas;
                                                                        if ([allscoCharge.payIndexValue isEqualToString:@"0"]) {
                                                                            paraValue = umpData;
                                                                        }
                                                                        [self openPayView:paraValue andCharge:allscoCharge];
 ...
 * 跳转支付界面
 */
- (void)openPayView:(NSString*)datas andCharge:(TAllScoCharge*)charge{
    if ([charge.payIndexValue isEqualToString:@"0"]) { // 银联支付
        [self doUPOMPPay:datas];
    } else if ([charge.payIndexValue isEqualToString:@"1"]) {
        [self doKQPay:datas and:charge.chargeAmount];
    }
}
...
-(void)payResult:(KQ_PayResult)result selfController:(UINavigationController*)controller {
    switch (result) {
        ...
        case KQ_PaySucceed:
            ...
            [[TServerFactory getServerInstance:@"TOrderServer"]doOrderSuccess:kqOrderString andParameter:_allscoCharge
...
- (void)dealloc {
    [super dealloc];
    [_chargeFormController release];
    _chargeFormController = nil;
}
